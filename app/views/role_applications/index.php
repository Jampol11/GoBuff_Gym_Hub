<?php
/**
 * Role Applications — Owner review list
 */
?>
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h4 class="mb-0 fw-bold">Role Applications</h4>
                <p class="text-muted mb-0 small">Review and assign roles to registered users.</p>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
            ...
            <ul class="nav nav-tabs border-0">
                <li class="nav-item">
                    <a class="nav-link <?= $statusFilter === '' ? 'active' : '' ?>"
                       href="<?= base_url('/role-applications') ?>">All</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= $statusFilter === 'pending' ? 'active' : '' ?>"
                       href="<?= base_url('/role-applications?status=pending') ?>">
                        Pending
                        <?php if ($pendingCount > 0): ?>
                ...
                <li class="nav-item">
                    <a class="nav-link <?= $statusFilter === 'approved' ? 'active' : '' ?>"
                       href="<?= base_url('/role-applications?status=approved') ?>">Approved</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= $statusFilter === 'rejected' ? 'active' : '' ?>"
                       href="<?= base_url('/role-applications?status=rejected') ?>">Rejected</a>
                </li>
            </ul>
        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <a href="<?= base_url('/role-applications/' . $app['id']) ?>"
        ...
        <div class="card-footer bg-transparent d-flex justify-content-end">
            <?= pagination_links($pagination, base_url('/role-applications')) ?>
        </div>
        <?php endif; ?>
    </div>
